 ...
 * Needs jsmin and cssmin (included in package)
 *
 * Ties into the 1.0.4 and up Yii CClientScript functions
 *
 * 0.9.2 Working @Import and relative paths with the help of a modified cssmin - Nafania
 * 0.9.1 Update of CssMin and JsMin to latest
...
class ExtendedClientScript extends CClientScript
{
	...
	*/
	...
	(
        'ImportImports'                 => true, // since 0.9.1
        'RemoveComments'                => true,
	...
	*/
	...
	/**
	 * Remove the JS files that has been set to be excluded
	 *
	 * @param <type> $output
	 * @param <type> $pos
	 */
	...
	private function combineAndCompress($type, $urls, $pos)
	{
		...
		if ($this->_renewFile)
		{
			...
			foreach ($urls as $key => $file)
			{
				...
				if ($type == 'js' && $this->compressJs && !$this->isMinified($file))
					$fileContents = $this->minifyJs($fileContents);

				if ($type == 'css' && $this->compressCss && !$this->isMinified($file))
				{
	...
	*/
	...
	/**
	 * Is given file minified already?
	 * 
	 * Only checks for presence of '.min' extension.
	 * @param string $file
	 * @return boolean
	 */
	private function isMinified($file) {
		return strpos(basename($file), '.min.') !== false;
	}
}